...
# 75% Training Data / 25 % Test Data SPLIT\n",
...
def preprocess_data(in_path, out_path):
    ...
    with (open(out_path + "/" + "Training_Flight_Data.csv", "wb")) as out_file:
        csv_writer_training = csv.writer(out_file)
        csv_writer_training.writerow(['arrival','distance','booking_date', 'city_status_destination',
        ...
        with (open(out_path + "/" + "Test_Flight_Data.csv", "wb")) as out_file_test:
            csv_writer_test = csv.writer(out_file_test)
            csv_writer_test.writerow(['arrival','distance','booking_date', 'city_status_destination',
                'direct', 'continent_source', 'departure', 'stops', 'city_status_source',
                'country_source', 'season', 'continent_destination', 'country_destination',
                'airline', 'nb_flights_offered', 'international', 'travel_date',
                'month_of_travel','days_until_departure','ticket_price'])
                ...
                if dir.startswith('Scrape') and not dir.endswith('.txt'):
                    for file in os.listdir(in_path + "/" + dir) :
                        if file.endswith(".json"):
                            ...
                            flight_data = []
                            sortedlist = sorted(json_data, key=lambda k: (k['travel_date'], k['ticket_price'], k['stops'], k['flight_duration']), reverse=False)

                            # Decide if write to training or test file
                            if decision(split_probability):
                                csv_writer = csv_writer_training
                            else:
                                csv_writer = csv_writer_test

                            for flight in sortedlist:
                                if flight['travel_date'] not in flight_data:
                                    try:
                                        ...
                                        if arrival_airportcode in airport_distances.index:
                                            ...
                                            flight_data.append(flight['travel_date'])
                                        else:
